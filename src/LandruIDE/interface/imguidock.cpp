...
namespace ImGuiDock
{
	...
	void updateDrag(lab::DockingWindow * window)
	{
		...
		if (split && split->active_dock)
		{
			...
			bool mouseleft = io.MouseDown[0];
			if (activeDock->dragging && mouseleft)
			{
				auto pos = io.MousePos;
				window->set_position((int) pos.x - 20, (int) pos.y - 20);
//				window->setAlpha(0.3f);
			}
			else
			{
				if (activeDock->dragging)
				{
